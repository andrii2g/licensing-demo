...
// Pure lifecycle state. Production clock is UTC + monotonic Stopwatch.
// Add internal clock injection when implementing deterministic gate tests.
internal sealed class LicenseGate
{
    ...
    {
        var monotonic = _budget - Stopwatch.GetElapsedTime(_startedAt);
        var wall = DateTimeOffset.FromUnixTimeSeconds(_expiresAt) - DateTimeOffset.UtcNow;
    ...
    public bool TryEnter(out IDisposable? admission)
    {
        ...
        {
            admission = null;
            if (_closed || RemainingLocked() <= TimeSpan.Zero)
                return false;
            _activeJobs++;
