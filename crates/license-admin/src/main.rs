...
fn run() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    if let Command::Keygen { directory, kid } = args.command {
        ...
        match d.read("issuer.key", 32, true) {
            ...
            _ => return Err("KEY_ALREADY_EXISTS".into()),
        }
        let seed = zeroize::Zeroizing::new(crypto::random::<32>()?);
        ...
        d.atomic(
            ...
            &serde_json::to_vec(&vec![(
                kid,
                envelope::encode(key.verifying_key().as_bytes()),
            )])?,
            false,
        )?;
        println!("KEY_CREATED");
    ...
    let dir = license_store::SecureDir::open_service(
        std::path::Path::new(&c.database_path)
            .parent()
            .ok_or("DATABASE_PATH")?,
    )?;
    let _lock = dir.lock()?;
    let mut db = license_server::repository::open(
        std::path::Path::new(&c.database_path),
    ...
    match args.command {
        ...
        Command::Entitlement {
            command: EntitlementCommand::Revoke { id },
        } => license_server::repository::revoke(&mut db, &id, now())?,
        Command::Installation {
            command: InstallationCommand::Retire { id },
        } => {
            let tx = db.transaction_with_behavior(rusqlite::TransactionBehavior::Immediate)?;
            let result = license_server::repository::retire(&tx, &id, now(), "admin")?;
            tx.commit()?;
            println!("{}", serde_json::to_string(&result)?);
        }
        Command::Installations {
            command: ListCommand::List { license_id },
        } => {
