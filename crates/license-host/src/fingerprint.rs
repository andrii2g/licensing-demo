...
pub fn machine_id(raw: &str) -> Result<String> {
    let v = raw
        ...
        .to_ascii_lowercase();
    if v.len() != 32 || !v.bytes().all(|b| b.is_ascii_hexdigit()) || v.bytes().all(|b| b == b'0') {
...
pub fn system_uuid(raw: &str) -> Result<String> {
    ...
    if v.len() != 36
        ...
        || v.bytes().filter(|b| *b != b'-').all(|b| b == b'0')
        || v.bytes().filter(|b| *b != b'-').all(|b| b == b'f')
    {
        return Err(Code::IdentityUnavailable);
    }
    Ok(v)
...
pub fn derive(product: &str, kind: &str, normalized: &str) -> String {
    ...
    mac.update(format!("{kind}:{normalized}").as_bytes());
...
mod tests {
    use super::*;
    #[test]
    fn supplied_fingerprint() {
        let v: serde_json::Value =
            serde_json::from_str(include_str!("../../../fixtures/fingerprint-vector.json"))
                .unwrap();
        for (kind, input, output) in [
            ("machine-id", "normalized_machine_id", "machine_id_hash"),
            ("system-uuid", "normalized_system_uuid", "system_uuid_hash"),
        ] {
            assert_eq!(
                derive("worker-suite", kind, v[input].as_str().unwrap()),
                v[output]
            );
        }
