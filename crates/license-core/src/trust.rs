use crate::{Code, Result, envelope::fixed};
    ...
    pub fn new(pairs: Vec<(String, String)>) -> Result<Self> {
        ...
        for (kid, key) in pairs {
            ...
            if !cfg!(any(test, feature = "dev"))
                && (kid.starts_with("TEST-") || key == FIXTURE_ISSUER || key == FIXTURE_DEVICE)
            {
                return Err(Code::UnknownKey);
            }
            let bytes = fixed::<32>(&key)?;
            let parsed =
                ed25519_dalek::VerifyingKey::from_bytes(&bytes).map_err(|_| Code::UnknownKey)?;
            if parsed.is_weak() || out.insert(kid, bytes).is_some() {
                return Err(Code::UnknownKey);
            }
        }
        Ok(Self(out))
    }
    pub fn key(&self, kid: &str) -> Result<&[u8; 32]> {
        self.0.get(kid).ok_or(Code::UnknownKey)
    }
    pub fn compiled() -> Result<Self> {
        let pairs: Vec<(String, String)> = crate::envelope::strict(
            include_bytes!(concat!(env!("OUT_DIR"), "/trust.json")),
            16384,
        )?;
        Self::new(pairs)
    }
