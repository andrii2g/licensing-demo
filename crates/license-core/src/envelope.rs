...
pub fn strict<T: DeserializeOwned>(bytes: &[u8], limit: usize) -> Result<T> {
    if bytes.is_empty() || bytes.len() > limit {
        return Err(Code::LicenseMalformed);
    }
    let mut de = serde_json::Deserializer::from_slice(bytes);
    let value = Strict(0)
        .deserialize(&mut de)
        .map_err(|_| Code::LicenseMalformed)?;
...
pub fn decode(s: &str, max: usize) -> Result<Vec<u8>> {
    if s.is_empty() || s.len() > max.div_ceil(3) * 4 {
        return Err(Code::LicenseMalformed);
    }
    let out = URL_SAFE_NO_PAD
        .decode(s)
        .map_err(|_| Code::LicenseMalformed)?;
    if out.len() > max || encode(&out) != s {
        return Err(Code::LicenseMalformed);
    }
    Ok(out)
}
...
impl Envelope {
    ...
    pub fn digest(&self) -> String {
        crate::crypto::hash(
            format!("{}.{}.{}", self.protected, self.payload, self.signature).as_bytes(),
        )
    }
