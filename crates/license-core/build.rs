fn main() {
    ...
    let data = match std::env::var("LICENSE_GUARD_TRUST_FILE") {
        Ok(path) => {
            ...
            std::fs::read_to_string(path).expect("read public trust file")
        }
        Err(_) => "[]".into(),
    };
    ...
    if std::env::var_os("CARGO_FEATURE_DEV").is_none() {
        for forbidden in [
            ...
            "5_FioQvsVZr-oZXk3OhLaVaNXSywlj60RsBoXisX8vA",
        ] {
            assert!(
                !data.contains(forbidden),
                "fixture trust forbidden in production"
            );
        }
    }
    std::fs::write(
        std::path::Path::new(&std::env::var("OUT_DIR").unwrap()).join("trust.json"),
        data,
    )
    .unwrap();
}
